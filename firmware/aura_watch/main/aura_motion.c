...
static void publish(float x, float y, float z, float movement, bool shook)
{
...
static void motion_task(void *unused)
{
    ...
    for (;;) {
        ...
        if (qmi8658_read_accel(&sensor, &x, &y, &z) == ESP_OK) {
            ...
            publish(filtered_x, filtered_y, filtered_z, movement, shook);
...
void aura_motion_init(void)
{
    ...
    qmi8658_enable_sensors(&sensor, QMI8658_ENABLE_ACCEL);
    portENTER_CRITICAL(&state_lock);
    state.available = true;
    portEXIT_CRITICAL(&state_lock);
